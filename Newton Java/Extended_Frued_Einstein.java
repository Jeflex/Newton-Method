...
public class Extended_Frued_Einstein {
	...
	public static Matrix F(double [][] x)
	{
		...
		for(int i = 0; i < n - 1; i ++){
	    if (i % 2 == 0){
				FF[i][0] = x[i][0] + (( 5 - x[i+1][0]) * x[i+1][0] - 2) * x[i+1][0] - 13;
			}

	    if (i % 2 == 1){
				FF[i][0] = x[i-1][0] + ((x[i][0] + 1) * x[i][0] - 14) * x[i][0] - 29;
			}
		}
	...
	public static Matrix J(double [][] x)
	{
		...
		for(i = 0; i < n; i++){
			if(i % 2 == 0){
				JJ[i][i] = 1;
				JJ[i][i+1] = 10 * x[i+1][0] - 3 * Math.pow(x[i+1][0], 2) - 2;
			}
			if(i % 2 == 1){
				JJ[i][i-1] = 1;
				JJ[i][i] = 3 * Math.pow(x[i][0], 2) + 2 * x[i][0] - 14;
			}
		}
